...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrwmean(), 'function', 'returns a function' );
	t.end();
});
...
tape( 'if not provided a weight, the accumulator function returns `NaN`', function test( t ) {
	var acc = incrwmean();
	t.strictEqual( isnan( acc( 2.0 ) ), true, 'returns NaN' );
	t.strictEqual( isnan( acc( 3.14 ) ), true, 'returns NaN' );
	t.end();
});

tape( 'if provided `NaN` for either a value or a weight, the accumulator function returns `NaN`', function test( t ) {
	var acc = incrwmean();
	t.strictEqual( isnan( acc( 2.0, NaN ) ), true, 'returns NaN' );
	t.strictEqual( isnan( acc( 3.14, NaN ) ), true, 'returns NaN' );

	acc = incrwmean();
	t.strictEqual( isnan( acc( NaN, 1.0 ) ), true, 'returns NaN' );
	t.strictEqual( isnan( acc( NaN, 1.0 ) ), true, 'returns NaN' );

	acc = incrwmean();
	t.strictEqual( isnan( acc( NaN, NaN ) ), true, 'returns NaN' );
	t.strictEqual( isnan( acc( NaN, NaN ) ), true, 'returns NaN' );
	t.end();
});
