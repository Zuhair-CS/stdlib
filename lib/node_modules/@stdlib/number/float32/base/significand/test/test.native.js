...
tape( 'the function returns a number', opts, function test( t ) {
	t.strictEqual( typeof significand( toFloat32( 3.14e30 ) ), 'number', 'returns a number' );
	t.end();
});
...
tape( 'the function returns the significand for `+-0`', opts, function test( t ) {
	t.strictEqual( significand( 0.0 ), 0, 'returns 0' );
	t.strictEqual( significand( -0.0 ), 0, 'returns 0' );
	t.end();
});

tape( 'the function returns the significand for `+infinity`', opts, function test( t ) {
	t.strictEqual( significand( PINF ), 0, 'returns 0' );
	t.end();
});

tape( 'the function returns the significand for `-infinity`', opts, function test( t ) {
	t.strictEqual( significand( NINF ), 0, 'returns 0' );
	t.end();
});
