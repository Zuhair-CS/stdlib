...
tape( 'the function tests for an array-like object containing only nonpositive numbers', function test( t ) {
	...
	t.strictEqual( isNonPositiveNumberArray( arr ), true, 'returns true' );

	arr = [ -5.0, '-3', null ];
	t.strictEqual( isNonPositiveNumberArray( arr ), false, 'returns false' );
...
tape( 'the function provides a method to test for an array-like object having only nonpositive number primitives', function test( t ) {
	...
	arr = [ -5.0, 0.0 ];
	t.strictEqual( isNonPositiveNumberArray.primitives( arr ), true, 'returns true' );

	arr = [ new Number( -5 ), -1.0, -1.0 ];
	t.strictEqual( isNonPositiveNumberArray.primitives( arr ), false, 'returns false' );
...
tape( 'the function provides a method to test for an array-like object having only nonpositive number objects', function test( t ) {
	...
	t.strictEqual( isNonPositiveNumberArray.objects( arr ), true, 'returns true' );

	arr = [ -5.0, 0.0 ];
	t.strictEqual( isNonPositiveNumberArray.objects( arr ), false, 'returns false' );
